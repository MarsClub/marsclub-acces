...
export const NIVEAUX: readonly Niveau[] = ['direction', 'management', 'equipe'] as const
 ...
 */
export function creerGarde<C extends string>(grille: Record<C, readonly Niveau[]>) {
  return {
    ...
    capacitesDe(niveau: Niveau): C[] {
      return (Object.keys(grille) as C[]).filter((c) => grille[c].includes(niveau))
    },
